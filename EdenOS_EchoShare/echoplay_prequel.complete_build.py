...
try:
    ...
    class YouTubePlaylistManager:
        def create_playlist_from_songs(self, *args, **kwargs): return None
...
class PlaylistApp(MDApp):
    ...
    def create_youtube_playlist(self, instance):
        ...
        threading.Thread(target=self._create_youtube_playlist_thread, daemon=True).start()
    ...
    def _create_youtube_playlist_thread(self):
        try:
            ...
            if playlist_id:
                ...
                Clock.schedule_once(lambda dt: self.show_success("YouTube playlist created! Opening in browser..."), 0)
                webbrowser.open(url)
            else:
                Clock.schedule_once(lambda dt: self.show_error("Failed to create YouTube playlist"), 0)
        except Exception as e:
            Clock.schedule_once(lambda dt: self.show_error(f"Error creating YouTube playlist: {e}"), 0)
        finally:
    ...
    def show_error(self, message: str):
        dialog = MDDialog(title="Error", text=message, buttons=[MDRaisedButton(text="OK", on_release=lambda x: dialog.dismiss())])
        dialog.open()

    def show_success(self, message: str):
        dialog = MDDialog(title="Success", text=message, buttons=[MDRaisedButton(text="OK", on_release=lambda x: dialog.dismiss())])
        dialog.open()
...
class PlaylistCLI:
    ...
    def run(self):
        ...
        while True:
            ...
            if choice == '1': self.generate_playlist_cli()
            elif choice == '2': self.load_ai_model()
            elif choice == '3': self.create_youtube_playlist_cli()
            elif choice == '4': self.save_playlist_cli()
            elif choice == '5': break
    ...
    def generate_playlist_cli(self):
        ...
        try:
            if AI_AVAILABLE and self.ai_curator.is_loaded:
    ...
    def create_youtube_playlist_cli(self):
        if not hasattr(self, 'current_playlist') or not self.current_playlist:
            ...
            return
        title = input("Title (blank = auto): ").strip() or f"Playlist - {datetime.now().date()}"
        privacy = input("Privacy [private/public/unlisted]: ").strip() or "private"
        pid = self.youtube_manager.create_playlist_from_songs(title=title, songs=self.current_playlist, privacy_status=privacy)
        if pid:
    ...
    def save_playlist_cli(self):
        if not hasattr(self, 'current_playlist') or not self.current_playlist:
            ...
            return
        fname = input("Filename: ").strip() or f"playlist_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        if not fname.endswith('.json'): fname += '.json'
        with open(fname, 'w', encoding='utf-8') as f:
            json.dump({"created": datetime.now().isoformat(), "songs": self.current_playlist}, f, indent=2)
