...
class AIDJApp:
    def __init__(self, master):
        ...
        self.query_label = tk.Label(self.search_frame, text="Song Query (e.g., 'Bohemian Rhapsody Queen'):")
        ...
        self.query_entry.bind("<Return>", self.search_music_from_event) # Allow pressing Enter

        self.search_button = tk.Button(self.search_frame, text="Search & Recommend", command=self.search_music)
        ...
        self.results_text = scrolledtext.ScrolledText(self.results_frame, wrap=tk.WORD, width=80, height=10)
        ...
        self.download_button = tk.Button(self.action_frame, text="Download Selected", command=self.prompt_download, state=tk.DISABLED)
        ...
        self.download_directory = os.path.join(os.path.expanduser("~"), "Downloaded_AI_Music")
    ...
    def _perform_search(self, query):
        ...
        try:
            result = subprocess.run(search_command, capture_output=True, text=True, check=True, encoding='utf-8', errors='replace')
            lines = result.stdout.strip().split('\n')
            
            if not lines or not lines[0]:
                self.master.after(0, lambda: self.update_results_text(f"No results found for '{query}'.\nTry a different query."))
                self.master.after(0, lambda: self.update_status(f"No results found for '{query}'."))
                return
            ...
            try:
                ...
                self.master.after(0, lambda: self.update_status(f"Found: '{title}' by '{uploader}'"))
                self.master.after(0, lambda: self.download_button.config(state=tk.NORMAL))

            except json.JSONDecodeError as e:
                self.master.after(0, lambda: self.update_results_text(f"Error parsing search result JSON: {e}\nRaw output:\n{lines[0][:500]}..."))
                self.master.after(0, lambda: self.update_status("Error parsing search result."))
    ...
    def prompt_download(self):
        ...
        if messagebox.askyesno("Download Confirmation", 
                               f"Do you want to download '{title}' by '{uploader}'?"):
            self.update_status(f"Starting download for '{title}'...")
            self.download_button.config(state=tk.DISABLED)
            # Run download in a separate thread
            threading.Thread(target=self._perform_download, args=(self.current_video_info,)).start()
        else:
    ...
    def _perform_download(self, video_info):
        ...
        if not webpage_url:
            self.master.after(0, lambda: self.update_status("Error: No URL available for download."))
        ...
        sanitized_uploader = "".join(c if c.isalnum() or c in (' ', '-', '_', '.') else '_' for c in uploader).strip()
        
        output_filename_template = os.path.join(self.download_directory, f"{sanitized_title} by {sanitized_uploader}.%(ext)s")
        ...
        try:
            ...
            self.master.after(0, lambda: self.update_status(f"Downloading '{title}'... (This may take a moment)"))
            process = subprocess.run(download_command, capture_output=True, check=True, text=True, encoding='utf-8', errors='replace')
            
            # You can parse process.stdout for progress if needed, but it's complex for single-file GUI.
            self.master.after(0, lambda: self.update_status(f"Successfully downloaded '{title}' to '{self.download_directory}'"))
            self.master.after(0, lambda: messagebox.showinfo("Download Complete", f"'{title}' downloaded successfully!"))
        except subprocess.CalledProcessError as e:
            ...
            self.master.after(0, lambda: messagebox.showerror("Download Error", error_message))
        except Exception as e:
            ...
            self.master.after(0, lambda: self.download_button.config(state=tk.NORMAL)) # Re-enable download button
