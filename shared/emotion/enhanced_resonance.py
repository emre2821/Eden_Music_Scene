...
class Resonance:
    ...
    def _find_meta_file(self, filename):
        ...
        if bundle_dir:
            ...
            potential_path = os.path.join(
                bundle_dir, "what_Eden_will_carry.chaosong.meta"
            )
        ...
        raise FileNotFoundError(
            f"Canonical pairings meta file not found: {filename}. "
                        ...
                        for line in pairing_block.split("\n"):
                            if ". " in line:
                                ...
                                if "&" in names:
                                    ...
                                    parts = names.split(" & ")
                                    primary_names = [
                                        n.strip() for n in parts[0].split(",")
                                    ]
                                    # In case the second part also has commas
                                    secondary_names = [
                                        n.strip() for n in parts[1].split(",")
                                    ]
        ...
        if self.has_emotion_analysis and self.emotion_decoder:
            try:
                if audio_path and os.path.exists(audio_path):
                    ...
                    emotional_metadata = (
                        self.emotion_decoder.generate_emotional_metadata(
                    ...
                    lyrical_emotions = self.emotion_decoder.decode_lyrical_emotions(
                        lyrics
                    )
                    pairing_emotions = self.emotion_decoder.suggest_pairing_emotions(
                        full_pairings, lyrical_emotions
                    )
                    ...
                    emotional_metadata = {
                        ...
                        "dominant_emotion": (
                            max(lyrical_emotions.items(), key=lambda x: x[1])[0]
                            if lyrical_emotions
                            else None
                        ),
                    }
    ...
    def generate_chaos_output(
        self, lyrics, audio_path=None, output_path="outputs/exports/resonance.chaos"
    ):
        """
            ...
            if "canonical_pairings" in metadata:
                ...
                for pairing, data in metadata["canonical_pairings"].items():
                    if "resonant_emotions" in data and data["resonant_emotions"]:
                        chaos_content += (
                            f"  {pairing}: {', '.join(data['resonant_emotions'])}\n"
                        )
        else:
...
if __name__ == "__main__":
    ...
    if result["emotional_metadata"]:
        print(
            f"Detected emotions: {result['emotional_metadata'].get('detected_emotions', {})}"
        )
