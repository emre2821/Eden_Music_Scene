    ...
    *,
    ...
    try:
        ...
        raise ConnectionError(
            f"Emotion service returned invalid JSON for {url}"
        ) from exc
    except error.HTTPError as exc:
        if exc.code == 404:
            return None
        raise ConnectionError(
            f"Emotion service returned HTTP {exc.code} for {url}"
        ) from exc
...
def get_tag(
    tag_id: str, *, base_url: Optional[str] = None, timeout: float = 5.0
...
def create_tag(
    tag: Dict[str, Any], *, base_url: Optional[str] = None, timeout: float = 5.0
) -> Dict[str, Any]:
    ...
    if result is None:
        raise ConnectionError(
            "Emotion service did not return a response for tag creation"
        )
    return result
