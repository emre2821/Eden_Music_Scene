...
class EmotionalMoment:
    ...
    emotional_journey: Optional[List[str]] = None  # Sequence of emotions in this session
...
class EmotionalMemory:
    ...
    def record_moment(self, 
                     ...
                     user_response: str,
                     interface_adaptations: Dict[str, any],
                     song_context: Optional[str] = None) -> None:
        """
        ...
        moment = EmotionalMoment(
            ...
            emotional_journey=[self._dominant_emotion(emotions) for emotions in journey]
        )
    ...
    def _update_patterns(self, moment: EmotionalMoment) -> None:
        ...
        if pattern_key in self.learned_patterns:
            ...
            # Create new pattern
            confidence = 0.3 if moment.user_response in ['positive', 'resonant'] else 0.1
    ...
    def predict_user_needs(self, detected_emotions: Dict[str, float]) -> Dict[str, any]:
        ...
        relevant_patterns = [
            pattern for key, pattern in self.learned_patterns.items()
    ...
    def get_emotional_journey_insights(self) -> Dict[str, any]:
        ...
        journey = [self._dominant_emotion(moment.detected_emotions) 
    ...
    def _assess_overwhelm_risk(self) -> bool:
        ...
        recent_responses = [moment.user_response for moment in self.session_moments[-3:]]
        negative_responses = sum(1 for response in recent_responses 
    ...
    def save_memory(self) -> None:
        ...
        memory_data = {
            ...
            'learned_patterns': {key: asdict(pattern) for key, pattern in self.learned_patterns.items()},
    ...
    def generate_memory_report(self) -> str:
        ...
        for emotion, patterns in by_emotion.items():
            ...
            for pattern in sorted(patterns, key=lambda p: p.confidence, reverse=True):
                report.append(f"  • {pattern.typical_response} response (confidence: {pattern.confidence:.1f})")
                report.append(f"    Seen {pattern.frequency} times, last: {pattern.last_seen[:10]}")
            report.append("")
