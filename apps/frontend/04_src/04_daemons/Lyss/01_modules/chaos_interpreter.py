...
from chaos_memory import load_relationships, save_relationships
from chaos_parser_core import parse_chaos_block

MEMORY_FILE = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "bond_graph.chaosmem"
)
ANTHEM_FILE = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "soulstitcher.awakening.chaosong"
)
...
def interpret():
    ...
    print(
        f"[SEAL] :: CHAOS Interpreter initialized and memory restored. ({datetime.utcnow().isoformat()})"
    )
    ...
    print(
        "[VEIL] :: distortion meta-mode invoked for 'My soul drifts in the ::distortion:: bet...' ("
    ...
    if target not in relationships[source]:
        ...
        print(
            f"[SEAL] :: Relationship forged: {source} ↔ {target} ({datetime.utcnow().isoformat()})"
        )
    else:
...
def parse_all_chaos_files(directory=None, recursive=False):
    ...
    pattern = (
        os.path.join(directory, "**", "*.chaos*")
        if recursive
        else os.path.join(directory, "*.chaos*")
    )
        ...
        try:
            ...
            with open(file, "r", encoding="utf-8") as f:
                ...
                if isinstance(results, list):
                    for item in results:
                        print(
                            f"  [ {item.get('tag', 'ECHO')} ] :: {item.get('value', '')}"
                        )
                elif hasattr(results, "echo_summary"):
                    print(results.echo_summary())
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        const=None,
        metavar="folder",
        help="Parse all .chaos* files in the specified folder (recursively)",
    )
    parser.add_argument(
        "--recursive",
        action="store_true",
        help="Recursively parse subfolders when using --parse-all",
    )
